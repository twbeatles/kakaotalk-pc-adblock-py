...
`dist\` is intentionally ignored and regenerated by `scripts\build_release.ps1`. Treat the latest local build command output, file size, and SHA256 as the current artifact proof rather than committing generated build outputs.
