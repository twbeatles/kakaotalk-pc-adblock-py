...
#### 스마트 최적화 결과 체계 (v10.x 핫픽스)
스마트 최적화는 내부적으로 3단계를 순차 실행하며, 결과를 `성공/부분 완료/실패`로 명확히 분류합니다.

| 상태 | 기준 | UI 알림 |
|:--|:--|:--|
| 성공 | Hosts 차단 성공 + DNS flush 성공 + 카카오톡 재시작 성공 | 완료 안내(`showinfo`) |
| 부분 완료 | Hosts 차단 성공 + (DNS flush 실패 또는 카카오톡 재시작 실패) | 경고 안내(`showwarning`) + 실패 단계 표시 |
| 실패 | Hosts 차단 실패 | 오류 안내(`showerror`) + 실패 단계 표시 |

> 안전성 정책: Hosts 차단 단계가 실패하면 DNS flush/재시작은 실행하지 않습니다.
...
### 스마트 최적화가 실패/부분 완료로 나와요
1. 앱을 **관리자 권한**으로 실행했는지 먼저 확인
2. 로그(`%APPDATA%\KakaoTalkAdBlockerPro\adblock.log`)에서 실패 단계 확인
3. 실패 단계별 조치
   - Hosts 실패: 보안 프로그램 잠금, 파일 권한, hosts 파일 손상 여부 확인
   - DNS 실패: `ipconfig /flushdns`를 관리자 CMD에서 수동 실행 후 재시도
   - 재시작 실패: 카카오톡이 백그라운드에 남아있는지 작업 관리자에서 확인 후 수동 실행

### hosts 파일 마커 손상 경고가 나와요
v10.x 핫픽스부터는 hosts 파일의 관리 블록 마커(`Start/End`)가 깨진 경우 **자동 수정하지 않고 안전중단**합니다.

- 이유: 시스템 hosts 파일의 비관리 영역을 잘못 손상시키는 위험을 차단하기 위함
- 복구 가이드:
  1. `C:\Windows\System32\drivers\etc\hosts`를 백업
  2. `# [KakaoTalk AdBlock Start]` ~ `# [KakaoTalk AdBlock End]` 블록을 수동으로 정리
  3. 저장 후 앱에서 [스마트 최적화]를 다시 실행
